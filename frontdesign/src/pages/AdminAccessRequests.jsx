                  ...
                  >
                    ...
                    {ch.status === 'pending' && (
                      <div className="flex gap-2">
                        <button
                          onClick={() =>
                            handleStatusUpdate(req._id, ch.chapterId, 'approved')
                          }
                          className="bg-green-600 text-white px-3 py-1 rounded hover:bg-green-700 text-sm flex items-center gap-1"
                          disabled={updatingId === `${req._id}-${ch.chapterId}`}
                        >
                          {updatingId === `${req._id}-${ch.chapterId}` ? (
                            <Loader className="animate-spin w-4 h-4" />
                          ) : (
                            <CheckCircle className="w-4 h-4" />
                          )}
                        ...
                        <button
                          onClick={() =>
                            handleStatusUpdate(req._id, ch.chapterId, 'rejected')
                          }
                          className="bg-red-600 text-white px-3 py-1 rounded hover:bg-red-700 text-sm flex items-center gap-1"
                          disabled={updatingId === `${req._id}-${ch.chapterId}`}
                        >
